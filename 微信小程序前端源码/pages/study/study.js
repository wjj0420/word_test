...
if (num == 0) {
  ...
  onLoad: function(){
    ...
    this.getFileURL(this.data.text[Index].english);
  },

  getFileURL:function(word){
  ...
  previous(){
    ...
    this.getFileURL(this.data.text[newIndex].english);
    this.data.innerAudioContext.stop() // 音频停止
      console.log("当前的索引值：",this.data.Index)
  },
  next(){
    ...
    this.getFileURL(this.data.text[nextIndex].english); // 获取下一张图片和音频
    this.data.innerAudioContext.stop() // 音频停止
